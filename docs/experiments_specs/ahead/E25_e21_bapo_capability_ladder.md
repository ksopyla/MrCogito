...
- **Status:** active (tiny DNA limits; 512 exclusive *concat slots* 0 bits at r=16/64/1;
  **raw override PASSES**; **r=1 in-place learned compressor 0 bits**; **inplace raw KV
  PASSES**; **inplace hard identity PASSES** — scatter is fine; learned `delta` wrecked
  r=1). Keep in `ahead/`. Do not score Glyph or 1024 yet.
...
- Verdict: **mixed.** Tiny INDEX near-pass at 8k. Tiny MATCH/select/chain walls. GPU seq=512
  ...
  bits). In-place **hard identity PASSES** (99.8% / 62.64 bits @750). Scatter is fine;
  learned `delta` wrecked r=1. Next: inplace r=16 frozen mean-pool.
...
Next ONE change: inplace r=16 with `--message_identity_slots` (frozen mean, no learned
`u`/`delta`), no raw_kv, same 512 `far_copy` recipe. Compression question on the trusted
mask. Not learned pool yet. Not 1024. Not Glyph.
