...
- Verdict: **mixed.** Tiny INDEX near-pass at 8k. Tiny MATCH/select/chain walls. GPU seq=512
  ...
  **0**. Default `--message_keep_local_swa` stays false. Next: parent
  may later map hops length **with extra hop** (not keepswa; not 320
  here).
...
FAIL]** without extra hops. Next ONE (do not run): parent may later map
hops length **with extra hop** (not keepswa; not 320 here). Do not
update_slot_kv / glob=3 / window 32. `--hops 1` is illegal. Not Glyph.
Do not unfreeze `u`/`delta`.
