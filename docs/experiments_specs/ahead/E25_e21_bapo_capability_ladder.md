...
- Verdict: **mixed.** Tiny INDEX near-pass at 8k. Tiny MATCH/select/chain walls. GPU seq=512
  ...
  **`--message_keep_local_swa` S1 PASS** (dense **94.8% / 23.53 bits**
  ...
  length with SWA kept (not 268 without the flag).
...
at glob=2 still **(264 S1 PASS, 272 FAIL]**. Next: **parent may later map hops
length with SWA kept** (not 268 without the flag; do not run here). `--hops 1`
is illegal. Not Glyph. Do not unfreeze `u`/`delta`.
