...
- **Status:** **ON HOLD** (queued behind [E05b](E05b_wd_confound_control.md), the wd-confound control, launched 2026-07-09). Run E05c only after E05b's wd verdict is in (so the non-bypassable objective is tested against the isolated collapse mechanism). Frozen spec — no changes pending the run.
