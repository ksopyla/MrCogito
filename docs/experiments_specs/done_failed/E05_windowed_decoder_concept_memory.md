...
- **Muon A/B (diverged, 2026-07-01):** Adam-vs-Muon optimizer A/B. Muon arms `concept_ar_prefix_H768L6C128D4_20260701_084351` (LR 0.02, [W&B](https://wandb.ai/ksopyla/MrCogito/runs/concept_ar_prefix_H768L6C128D4_20260701_084351)) + `..._20260701_194042` (LR 0.01, [W&B](https://wandb.ai/ksopyla/MrCogito/runs/concept_ar_prefix_H768L6C128D4_20260701_194042)) · Run report [e05_muon_divergence_rootcause_20260701.md](../../2_Experiments_Registry/run_reports/e05_muon_divergence_rootcause_20260701.md). **Verdict: Muon diverges on E05 at both LRs (delayed onset ~step 3k/4.5k) — but converges ~5× faster first** (eval_loss **3.34** at step 4k vs Adam's ~5.40). Root cause: Muon's full-rank orthogonalized updates grow weight spectral norms (Q·K + lm_head bilinear couplings) with **no weight decay** to curb it + over-hot `adamw_lr=2e-3`; survives `max_grad_norm=0.5`. Mitigations implemented (`WEIGHT_DECAY=0.1`, `MUON_ADAMW_LR=2e-4`, sustained-LR `constant_with_warmup` calibration). **Retry SUCCEEDED (2026-07-02):** run `concept_ar_prefix_H768L6C128D4_20260702_031956` completed 0.5 ep with eval_loss **2.606** (vs Adam 3.83 — ~5× faster convergence + 1.22 nats lower), stable end-to-end (grad_norm ~0.5–1.6, no divergence). **Eval (2026-07-04, ⚠️ TENTATIVE): split — optimization win, concept-bottleneck regression.** Downstream semantics up (STS-B **0.518** clearing both floors vs Adam 0.452; SICK-R 0.302, GLUE MRPC 0.725 / STSB 0.532 all > Adam), BUT concept geometry + long-range gate regressed hard (within-sample RankMe **10.57** vs Adam 37.67; Δshuffle_beyond **0.209** vs 0.39 — fails the Stage-1 floor ≥0.3; Δzero_beyond 0.41 vs 6.99) → the lower loss came from the decoder's within-window bypass, not richer concepts. **Not decisive** — open: wd confound (Muon wd=0.1 vs Adam 0.0, needs Adam@0.1 to isolate); authoritative prefix→suffix `concept_ablation/*` in W&B not yet read (reconstruction-contract numbers above may understate); single seed/run. Discussion + literature pending.
- **Muon long 2-ep (2026-07-09):** Run id `concept_ar_prefix_H768L6C128D4_20260704_225659` · WandB [Link](https://wandb.ai/ksopyla/MrCogito/runs/concept_ar_prefix_H768L6C128D4_20260704_225659) · Run report [e05_muon_long_2ep_collapsed_20260709.md](../../2_Experiments_Registry/run_reports/e05_muon_long_2ep_collapsed_20260709.md). **Verdict: REGRESSION — more compute collapsed the bottleneck harder.** Compute-matched to E02-long (**300.88 GPU-h / 85.25 kWh / 40.78 B tokens**, 2 ep / 276566 steps), stable end-to-end (grad_norm 0.4–0.8 mid, ~3.7 cosine tail, no divergence), eval_loss fell to a project-low **2.581** (step 272000). But **every concept gate regressed vs the 0.5-ep Muon arm**: within-sample RankMe **4.96/128** (was 10.57; centered 4.61 → genuine collapse); Δshuffle_beyond **0.227** (was 0.209; flat from step 4k in W&B `concept_ablation/*`); **STS-B zero-shot 0.062** (was 0.518 — now 0.42 *below* the token-embed-mean floor). Slot-mean 1.66, mean concept cosine 0.892. **Falsifies the "more training de-collapses it" hypothesis for the windowed+Muon regime** — the K=128 within-window bypass is the attractor; extra optimization makes the bypass better, not the concepts richer (opposite of E02-long's full-causal de-collapse). eval_loss is orthogonal to concept quality (loss ↓ while RankMe ↓ and STS-B ↓). **Closes the E05 from-scratch track's "more compute" branch.** Open confounds (do not block the pivot): wd confound (Muon wd=0.1 vs Adam 0.0); Tier-1 protocol split (0.5-ep old / 2-ep new protocol — seq-512 flattens collapse, so the regression holds a fortiori). Reinforces the E10 pretrained-backbone pivot. **Mechanism deep-dive (2026-07-09):** no weight corruption (0 NaN/Inf, 229 tensors); collapse = rank-1 of `enc.L5.bixt.rv_lat` (834/1536 dead rows) — 128 diverse slots all fed the same single-rank document summary. The LR=0.003 grad-norm-rise/loss-fall event is an Edge-of-Stability gorge descent (descendable-sharpness ceiling `2/(η·s)` lifts as η falls); wd is the proximate collapse driver (selective shrinkage of bypass-redundant directions, `muon.py:101`). Targeted follow-ups (cheap, not a broad re-investment): Adam@wd=0.1 control [E05b](../done_success/E05b_wd_confound_control.md) + anti-collapse objective extension [E05c](../ahead/E05c_anticollapse_extension.md); lit [concept_bottleneck_collapse_mitigation.md](../../literature_review/concept_bottleneck_collapse_mitigation.md).
