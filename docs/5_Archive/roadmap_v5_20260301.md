> **OBSOLETE — 2026-06-05**
> Superseded by **[agenda.md](../1_Strategy_and_Plans/agenda.md)** (the slim living agenda) + per-experiment specs in **[docs/experiments_specs/](../experiments/)**.
> **Why:** The 8-track / 6-phase plan blended too many experiments at once and read as a committed schedule. The project moved to a lighter process — small, well-defined increments tracked in `agenda.md` with per-experiment specs — while the long-term direction still lives in `vision_and_goals.md`. This monolithic roadmap is kept only as history.
> Kept for historical reference; all current work follows the links above.
...
**Detailed speech-to-speech research notes:** [concept_model_speech2speech.md](../literature_review/concept_model_speech2speech.md)
