...
| **capability suite** | the standard exams every new architecture runs: 7 levels of increasing difficulty (learns at all → carries a fact → picks signal over a lookalike → long reach → multi-step reasoning → language-like noise → BAPO-hard stretch) at 5M / 10M / 30M / 50M, scored against recorded past architectures. Spec `docs/engineering_specs/capability_suite.md`. |
