# E25 bridge seq=384 chain_ordered --key_len 13 `--global_layers 2` — dense K1 (rung 5bb)

**Date:** 2026-09-14
**Machine:** Odra 1× RTX 3090 (GPU 0). Polonez left empty. E22 Byobu sessions untouched.
**Run ID:** `e25_384_chain_k13_glob2` (800 advertised; dense K1 budget 3200; e18/e21/`e18_local` skipped; 8k not run)
**WandB:** n/a (probe; no `compute/*` audit — there is no W&B run)
**Raw JSON/plots:** `/opt/cursor/artifacts/e25_bridge384_chain_k13_glob2/`
**Raw log:** `/opt/cursor/artifacts/e25_bridge384_chain_k13_glob2/probe.log`
**Best checkpoint:** none (on-the-fly rows; models discarded)
**Git commit:** `3aca9b9` (Odra E24 worktree; E22 main checkout untouched)
**Git tag:** —
**Related:** seq=256 glob2 S1 PASS [`e25_bridge256_chain_k13_glob2_20260914.md`](e25_bridge256_chain_k13_glob2_20260914.md) · seq=512 glob2 K1 [`e25_bridge512_chain_k13_glob2_20260914.md`](e25_bridge512_chain_k13_glob2_20260914.md)
...
live E18 **5.56 bits**. Same recipe at seq=512 was dense **K1** (22.5% / 0 bits)
at both glob=1 and glob=2. Hypothesis: packed 384 (midpoint of the dense hops
wall) is dense-solvable at two global Blocks. Same DNA + compressor as the 256
glob2 PASS, **only** `--seq_len 384` (bridge default is 512). Recalibrate dense
S0 at two global blocks in this JSON.
...
| Data | `--scale bridge --seq_len 384`, **`--key_len 13`**, hops=2, **26-bit** prize |
| Geometry | min_gap **64** > local_window **16**. Right-align gap 220/246/297 |
| Init | `--warm_residuals` |
| Device | CUDA bf16 (`--amp auto`), `backend=sdpa` · 0.05 s/step, no OOM |

```
bash scripts/e24_bapo_hunt.sh e25_384_chain_k13_glob2 0 \
  --scale bridge --seq_len 384 --recipe chain_ordered --arch dense e18 e21 e18_local \
...
# --seq_len 384 required (bridge default is 512)
...
Byobu `E25_384_chain_glob2`. Log:
`seq=384  gap=64  window=16  prize=26.00 bits  answer_len=13
row_gap[min/med/max]=220/246/297`
dense `patterns=[('full', 0)×5]  glob_layers=2` · 2.802M.

Hunt JSON: `seq_len: 384`, `min_gap: 64`, `local_window: 16`, `hidden: 256`,
...
| gate | result |
|---|---|
| **S0** | **FAIL / K1.** Dense 24.5% / 0 bits @3200 < 75%. Recalibrated at `global_layers=2`. |
...
**Two sequential global attend+FFN Blocks do not make packed 384 hops
dense-solvable.** The same compressor and glob=2 recipe that composed hops at
seq=256 (dense 25.46 / E21 25.85) is chance at seq=384 for dense — matching
seq=512 glob=2 K1 (22.5% / 0 bits). The dense hops wall at glob=2 is **context
length**, not a missing midpoint between 256 and 512. Wall is now **(256 S0
PASS, 384 K1]**.
...
`--global_layers` stays 1. Next ONE (do not run): seq=**320** packed
`chain_ordered --key_len 13` `--global_layers 2` (localize the dense hops wall
**(256 S0 PASS, 384 K1]**). Recalibrate dense S0. `--hops 1` is illegal.
