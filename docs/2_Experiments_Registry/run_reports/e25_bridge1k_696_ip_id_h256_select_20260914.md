# E25 bridge_1k seq=696 select_1decoy r=1 identity H=256 SSMax log — E21 vs E18 vs dense (rung 5at)

**Date:** 2026-09-14
**Machine:** Odra 1× RTX 3090 (GPU 0). Polonez left empty. E22 Byobu sessions untouched.
**Run ID:** `e25_696_ip_id_h256_select` (800; floor — do not extra-step)
**WandB:** n/a (probe; no `compute/*` audit — there is no W&B run)
**Raw JSON/plots:** `/opt/cursor/artifacts/e25_bridge1k_696_ip_id_h256_select/`
**Raw log:** `/opt/cursor/artifacts/e25_bridge1k_696_ip_id_h256_select/probe.log`
**Best checkpoint:** none (on-the-fly rows; models discarded)
**Git commit:** `fdefa6e` (Odra E24 worktree; E22 main checkout untouched)
**Git tag:** —
**Related:** 688 SELECT identity PASS [`e25_bridge1k_688_ip_id_h256_select_20260914.md`](e25_bridge1k_688_ip_id_h256_select_20260914.md) · 704 SELECT identity chance [`e25_bridge1k_704_ip_id_h256_select_20260914.md`](e25_bridge1k_704_ip_id_h256_select_20260914.md) · 672 SELECT identity PASS [`e25_bridge1k_672_ip_id_h256_select_20260914.md`](e25_bridge1k_672_ip_id_h256_select_20260914.md)
...
(688 needed extra-step 8k after a 31.57-bit climb @800). Seq=704 / 768 /
1024 identity chance. E18 still copies at 672/688/696/704/1024. This
rung **tightens SELECT length** at seq=**696** with the **same 1024-passing
...
```
bash scripts/e24_bapo_hunt.sh e25_696_ip_id_h256_select 0 \
...
Byobu `E25_696_id_select`. Log:
`seq=696  gap=64  window=16  prize=64.00 bits  answer_len=32  logit_scale=log
...
`test_bridge_1k_seq_len_696_select_keeps_window_below_gap`.
...
| e18 | **100%** | 350 | **63.93** | 0.999 | 0.01148 |
| **e21 @800** | **25.0%** | 800 | **0.00** | **0.000** | **1.46e-7** |
| e18_local | 24.7% | 800 | 0.00 | 0.000 | 0 |

Params: dense/e18 ~2.261M, e21 2.277M (<100M). Prize 64 bits. Chance ~25%.
E21 CE at ln(4)≈1.386 every eval (min 1.3860, max 1.3948; acc never >
0.262). Best acc 26.2% = chance. Not climbing. E18 live **100% / 63.93
bits** @350 (do not pass S1 via 0.75×0). Dense early-stop 100% @200. Hunt
exit 0. **8k extra-step did not run.**

S1 vs 0.75× this-JSON E18: need **47.95 bits**. E21 has **0.00**. Content vs
...
| **S1 vs 0.75× this-JSON E18** | **FAIL.** 0.00 ≪ 47.95 bits. Chance floor. |
...
**Exclusive identity SELECT that passed at 688 is chance at 696** on the
...
chance). Unlike 688, the 800-step snapshot is a floor, not a mid-click dip,
so extra-step 8k is not justified. The SELECT length wall is now **(688
PASS, 696 FAIL]**. 704 chance was the same wall, not a later one. Do **not**
relabel E18's 63.93 bits as E21.

## Decision

Keep the spec in `ahead/`. Defaults stay: remainder off, extra hops 0,
`update_slot_kv` off, `keep_local_swa` off, `global_layers` 1, anchors
`none`. Next ONE (do **not** run): seq=**692** packed `select_1decoy` with
the **same** 1024-passing recipe (`--scale bridge_1k --seq_len 692 --hidden
...
none). Tightens the SELECT length wall in (688, 696]. Not architecture
