# E25 bridge seq=512 chain_ordered --key_len 13 `--global_layers 2` — dense K1 (rung 5ba)

**Date:** 2026-09-14
**Machine:** Odra 1× RTX 3090 (GPU 0). Polonez left empty. E22 Byobu sessions untouched.
**Run ID:** `e25_512_chain_k13_glob2` (800 advertised; dense K1 budget 3200; e18/e21/`e18_local` skipped; 8k not run)
**WandB:** n/a (probe; no `compute/*` audit — there is no W&B run)
**Raw JSON/plots:** `/opt/cursor/artifacts/e25_bridge512_chain_k13_glob2/`
**Raw log:** `/opt/cursor/artifacts/e25_bridge512_chain_k13_glob2/probe.log`
**Best checkpoint:** none (on-the-fly rows; models discarded)
**Git commit:** `34e58b5` (Odra E24 worktree; E22 main checkout untouched)
**Git tag:** —
**Related:** glob=1 512 K1 [`e25_bridge512_chain_k13_20260914.md`](e25_bridge512_chain_k13_20260914.md) · seq=256 glob2 S1 PASS [`e25_bridge256_chain_k13_glob2_20260914.md`](e25_bridge256_chain_k13_glob2_20260914.md)
...
live E18 **5.56 bits**. Same recipe at seq=512 with **one** global layer was
dense **K1** (22.5% / 0 bits). Hypothesis: two sequential global attend+FFN
Blocks that compose hops at 256 also make packed 512 dense-solvable. Same DNA
+ compressor as the 256 glob2 PASS, **only** seq=512 (bridge default; no
`--seq_len` override). Recalibrate dense S0 at two global blocks in this JSON.
...
| Data | `--scale bridge` seq=**512** (scale default), **`--key_len 13`**, hops=2, **26-bit** prize |
| Geometry | min_gap **64** > local_window **16**. Right-align gap 284/321/425 |
| Init | `--warm_residuals` |
| Device | CUDA bf16 (`--amp auto`), `backend=sdpa` · ~1.8GB, no OOM |

```
bash scripts/e24_bapo_hunt.sh e25_512_chain_k13_glob2 0 \
...
# no --seq_len (bridge default is 512)
...
Byobu `E25_512_chain_glob2`. Log:
`seq=512  gap=64  window=16  prize=26.00 bits  answer_len=13
row_gap[min/med/max]=284/321/425`
dense `patterns=[('full', 0)×5]  glob_layers=2` · 2.802M.

Hunt JSON: `seq_len: 512`, `min_gap: 64`, `local_window: 16`, `hidden: 256`,
...
| gate | result |
|---|---|
| **S0** | **FAIL / K1.** Dense 22.5% / 0 bits @3200 < 75%. Recalibrated at `global_layers=2`. |
...
**Two sequential global attend+FFN Blocks do not make packed 512 hops
dense-solvable.** The same compressor and glob=2 recipe that composed hops at
seq=256 (dense 25.46 / E21 25.85) is chance at seq=512 for dense — matching
glob=1 K1 (22.5% / 0 bits, 2.261M). The 512 hops wall is **context length**,
not a missing second global Block, not key packing (13 vs 24), not width
(H=128 / H=256 log / H=512 MHA already failed). Dense hops wall at glob=2 is
**(256 S0 PASS, 512 K1]**.
...
Code default `--global_layers` stays 1. Next ONE (do not run): seq=**384**
