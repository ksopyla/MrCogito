...
- **Run state (2026-06-30):** long-context engineering rounds 1–2 done (F1–F4, F2′, F6 sequence parallelism → 1M on 3× 3090; F7 Muon). **E05 attempt 3 (`_20260629_093840`) TRAINING COMPLETE — 0.5 ep / 10.2B tokens / 68.2 GPU-h / 18.24 kWh, no divergence.** Batch 8 × accum 3 × 3 GPU = effective 72 (batches 12 / 10 both OOM'd during the loss FP32 upcast — see run report). eval_loss fell monotonically 5.40 → 3.83 across 17 evals; pre-clip grad_norm held 0.4–0.55 through step ~48k, then rose to 40–75 during the cosine-tail region (LR ≈ 1e-6) without hurting eval_loss — opposite of attempt 2's escalation. Best checkpoint-68000 (eval_loss 3.829). Stage 1 read (geometry, Δshuffle_beyond, STS-B) pending eval. **E05 attempt 2 (`_20260627_192407`) DIVERGED at step 40k** — kept as the divergence reference; see attempt 2 run report.
...
- **E05 attempt 3 — windowed decoder, completed 0.5 ep (Odra, eval pending 2026-06-30):** the LR 5e-5 / clip 0.5 retune of attempt 2 **held — first fully completed E05 run**. 0.5 ep / 10.2B tokens / 68.2 GPU-h / 18.24 kWh (cheaper per-token than attempt 2's wasted 81.3 GPU-h, and 2× the tokens). eval_loss fell monotonically 5.40 → 3.83 across 17 evals (Δ shrank to −0.001 by the end — clean convergence). grad_norm held 0.4–0.55 through step ~48k, then rose to 40–75 in the cosine-tail region (LR ≈ 1e-6) without hurting eval_loss — the *opposite* signature from attempt 2's escalation. Batch 8 × accum 3 × 3 GPU = effective 72 (batches 12 and 10 both OOM'd on the loss FP32 upcast at seq 2048 + K=128). Best checkpoint-68000 (eval_loss 3.829). **Takeaway:** the divergence was indeed optimization-side, not architectural — Stage 1 read (geometry, Δshuffle_beyond, STS-B) will tell whether the windowed bottleneck earns the matched A/B budget. See [run report](../2_Experiments_Registry/run_reports/e05_attempt3_completed_20260630.md).
