...
## Current focus
- **Attack concept collapse at the root, on the encoder→decoder foundation.** E01–E04 are done and evaluated. E02-long (STS-B 0.714, RankMe 246) remains the semantic leader; E03 anchor helps at 0.3 ep recon; **E04 (parallel decoder, eval 2026-06-20)** shows within-sample RankMe 108 and cross-sample RankMe 178 (+27 vs E03 control) but STS-B 0.532 — bypass removal correlates with richer geometry, not E02-level semantics.
- **Run state (2026-06-20):** E04 DONE on Odra (`perceiver_denoise_H768L6C128D4_20260618_200645`, ck-66000). **Odra free.** Next candidates: (1) Tier-2.5 attention-pool probe on E04 / E02-long; (2) prefix→suffix + anchor; (3) E05 windowed decoder (foundation ready, gate cleared); (4) decoder-weakening ablation (`DECODER_WORD_DROPOUT=0.5`).
- **Queued (sequential — one experiment per server):**
  ...
  3. **Prefix→suffix + anchor (highest-value E03 follow-up):** combine the two de-collapse levers on the stronger objective. Needs a one-line spec amendment (objective=prefix_suffix + anchor_loss=true).
  4. **E05 windowed decoder** — [(spec)](../experiments_specs/E05_windowed_decoder_concept_memory.md) · foundation implemented; E04 gate cleared.
...
5. **E05 — windowed decoder + concepts as cross-window memory** [(spec)](../experiments_specs/E05_windowed_decoder_concept_memory.md) · [(plan)](../experiments_specs/E05_windowed_decoder_concept_memory_plan.md)
   ...
   **Scoped to seq-len 2K + a long-doc dataset mix** (`DATASET_MIX=e05_long_2k`: FinePDFs 0.50 / FineWeb-Edu 0.30 /
...
- **E04 — parallel Perceiver-IO decoder, reconstruction (FineWeb-Edu, Odra, eval 2026-06-20):** removes the AR bypass (no token self-attention). **Within-sample RankMe 107.8**, cross-sample RankMe 177.8 (+27 vs E03 control, clears +8 gate); STS-B 0.532 > control 0.485 but << E02 0.702. Slot rank ~11 (E02-like). Early-Δ unscored (parallel decoder). Directional support that bypass removal enriches per-input concept geometry without closing the semantic gap vs prefix→suffix. Confounded by 1 ep vs 0.3 for E03 control and decoder-family change. See [run report](../2_Experiments_Registry/run_reports/e04_parallel_decoder_20260620.md).
