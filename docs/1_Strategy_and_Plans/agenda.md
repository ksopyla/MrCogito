...
- **Run state (2026-06-28):** long-context engineering rounds 1–2 done (F1–F4, F2′, F6 sequence parallelism → 1M on 3× 3090; F7 Muon). **E05 attempt 2 (`_20260627_192407`) DIVERGED at step 40k / epoch 0.19 / 5.2B tokens** — eval_loss climbed 3.32 → 4.03, pre-clip grad_norm escalated 9 → 903 while cosine LR was still ~8.5e-5. Best checkpoint-40000 preserved (within-sample RankMe 59.8, Δshuffle_beyond 0.35 — Stage 1 floor cleared). Architecture is sound; divergence is optimization-side. **Retuned and re-scoped to 0.5 epoch (~7B tokens, ≈ E02-long's 5-ep compute-hours):** LR 5e-5 (was 1e-4), `max_grad_norm` 0.5 (was default 1.0, now wired through launcher), per-device batch 12 (was 8), warmup 2000, eval_steps 4000. Attempt 3 next on Odra.
