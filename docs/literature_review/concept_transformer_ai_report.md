...
- Using the encoder's final layer outputs corresponding to C specific "concept token" positions (the exact nature of these positions is not fully specified, but they result in a tensor of shape [batch_size,concept_size,hidden_size]) as the Queries in the cross-attention mechanism.
...
A crucial and challenging aspect is defining how the concept representation [batch_size,concept_size,hidden_size] is mapped back to sequence-level logits [batch_size,sequence_len,vocab_size] to compute the loss for the MLM training objective. The user suggests a specific two-step mapping:
